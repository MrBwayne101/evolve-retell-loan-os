...
def evaluate_compliance_gate(
  ...
  *,
  ...
  timezone_name = _coalesce_text(lead.get("timezone"), "America/Chicago")
  ...
  if not _is_within_dial_window(local_time):
...
def _detect_dnc_flag(
  ...
  if explicit is True:
    return True
  dnc_tags = {"dnc", "do not call", "do-not-call", "stop calling"}
...
def _next_allowed_dial_time(local_time: datetime) -> datetime:
  ...
  if _is_within_dial_window(local_time):
    return local_time
  if local_time.hour > 20 or (local_time.hour == 20 and local_time.minute > 0):
    next_day = (local_time + timedelta(days=1)).date()
    return datetime(next_day.year, next_day.month, next_day.day, 8, 0, tzinfo=local_tz)
  return datetime(local_time.year, local_time.month, local_time.day, 8, 0, tzinfo=local_tz)
